...
int bmp180_init(bmp180_t *dev, i2c_t i2c, uint8_t mode) {
	...
	if(i2c_read_regs(dev->i2c_dev, BMP180_ADDR, BMP180_CALIBRATION_AC1, buffer,
			22) < 0) {
...
void bmp180_auto_init(void) {
	for(unsigned i = 0; i < BMP180_NUMOF; i++) {
		if(bmp180_init(&bmp180_devs[i], bmp180_params[i].i2c_dev,
...
int bmp180_sealevel_pressure(bmp180_t *dev, int32_t altitude,
		int32_t *pressure_0) {
	int32_t p;
	bmp180_read_pressure(dev, &p);
...
static int _read_ut(bmp180_t *dev, int32_t *output) {
	...
	{ 0 };
	uint8_t control[2] =
	{ BMP180_REGISTER_CONTROL, BMP180_TEMPERATURE_COMMAND };
...
static int _read_up(bmp180_t *dev, int32_t *output) {
	...
	BMP180_PRESSURE_COMMAND | (dev->oversampling & 0x3) << 6 };
	i2c_write_bytes(dev->i2c_dev, BMP180_ADDR, control, 2);
