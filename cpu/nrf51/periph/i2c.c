...
int i2c_read_bytes(i2c_t dev, uint8_t address, void *data, int length)
{
    ...
    transfer_succeeded = twi_master_transfer(address|TWI_READ_BIT, data, length, TWI_ISSUE_STOP);
...
{
    bool transfer_succeeded;
    transfer_succeeded  = twi_master_transfer(address, &reg, 1, TWI_DONT_ISSUE_STOP);
    transfer_succeeded &= twi_master_transfer(address|TWI_READ_BIT, data, length, TWI_ISSUE_STOP);
    if (transfer_succeeded) {
    	return length;
    }
...
{
    bool transfer_succeeded;
    transfer_succeeded = twi_master_transfer(address, (void *)data, length, TWI_ISSUE_STOP);
    if (transfer_succeeded) {
    	return length;
    }
    return -1;
