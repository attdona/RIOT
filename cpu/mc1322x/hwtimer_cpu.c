...
void timer_x_init(volatile struct TMR_struct *const TMRx)
{
    ...
    TMRx->CTRLbits.PRIMARY_CNT_SOURCE =
        0x0f;   /* Perip. clock with 128 prescale (for 24MHz = 187500Hz) */
    ...
    TMRx->CTRLbits.CO_INIT =
        0x00;              /* other counters cannot force a reinitialization of this counter*/
...
void hwtimer_arch_set(unsigned long offset, short timer)
{
    ...
    tmr->COMP1 = tmr->CNTR +
                 offset;            /* load the current value + offset into the compare register */
    ...
    tmr->SCTRLbits.TCFIE =
        1;                   /* enable interrupts when TCF is one  - do we need both?*/
