...
#define PHY_TIMER_MASK 0x00000000ffffffff

//#define TIMER_MAX_VALUE (1<<32)
...
static uint64_t _time_left(uint64_t target, uint32_t reference) {
...
static void _add_timer_to_list(xtimer_t **list_head, xtimer_t *timer) {
    ...
    *list_head = timer;
    ...
    if (timer_list_head == timer) {
        if ((timer->target & PHY_TIMER_MASK) == timer->target) {
...
static void _timer_callback(void) {
    ...
    reference = xtimer_now();

    in_the_past:
    /* check if next timers are close to expiring */
    while (timer_list_head
            && (_time_left(timer_list_head->target, reference)
                    < XTIMER_ISR_BACKOFF)) {
    ...
    if (timer_list_head
            && (timer_list_head->target & PHY_TIMER_MASK)
                    == timer_list_head->target) {
...
static inline void _timer_wrap(void) {
    ...
    while (list_head) {
        ...
        list_head->target = list_head->target - ((uint64_t) (1) << 32);
        ...
        if (reference > list_head->target) {
            ...
            if ((timer_list_head->target & PHY_TIMER_MASK)
                    == timer_list_head->target) {
...
static void inline _xtimer_set_absolute(xtimer_t *timer, uint64_t ticks_offset)
{
    // internal units are clock ticks
    timer->target = ticks_offset;

    _add_timer_to_list(&timer_list_head, timer);

}

void inline _xtimer_set64(xtimer_t *timer, uint64_t offset) {
    uint32_t now = xtimer_now();

    // internal units are clock ticks
    _xtimer_set_absolute(timer, now + offset * USEC_TICKS);
}
...
void xtimer_periodic_wakeup(uint32_t *last_wakeup, uint32_t period) {
    ...
    uint32_t target = (*last_wakeup) + (period * USEC_TICKS);
    uint32_t now = xtimer_now();
...
void xtimer_set_msg(xtimer_t *timer, uint32_t offset, msg_t *msg,
    ...
    xtimer_set_msg64(timer, offset, msg, target_pid);
}
 ...
 * @param[in] target_pid    pid the message will be sent to
 */
void xtimer_set_msg64(xtimer_t *timer, uint64_t offset, msg_t *msg,
                      kernel_pid_t target_pid) {

    timer->target = xtimer_now() + offset * USEC_TICKS;
...
int xtimer_msg_receive_timeout64(msg_t *msg, uint64_t us) {
    ...
    xtimer_set_msg64(&t, us, &tmsg, sched_active_pid);
...
int xtimer_msg_receive_timeout(msg_t *msg, uint32_t us) {
    return xtimer_msg_receive_timeout64(msg, us);
}
